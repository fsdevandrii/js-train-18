...
function checkData(data) {
  ...
  if (Object.keys(data).length === 0) {
    return "Об'єкт пустий"; // Повертаємо рядок, якщо об'єкт порожній
  } else {
    return data; // Повертаємо дані, якщо об'єкт не є порожнім
  }
}
...
function parseJson(jsonStr) {
  ...
  try {
    ...
    // Якщо виникла помилка, повертаємо повідомлення про помилку.
...
function getAge(age) {
  ...
  try {
    ...
    if (age < 0) {
      // Якщо вік менше 0, генеруємо помилку з властивістю name і текстом "Вік не може бути менше 0!"
      const error = new Error("Вік не може бути менше 0!");
      error.name = "AgeError";
      throw error;
    }
    // Якщо вік коректний, повертаємо рядок з віком користувача
    return `Вік користувача: ${age}`;
  } catch (error) {
    // Якщо виникла помилка, повертаємо об'єкт з властивостями name та message помилки
...
function getBookById(books, id) {
  ...
  try {
    ...
    // Якщо книга не знайдена, генеруємо TypeError з відповідним повідомленням.
    if (!book) {
      throw new TypeError(`Книга з ID ${id} не знайдена!`);
    }

    // Якщо книга знайдена, повертаємо її назву.
    return `Книга: ${book.title}`;
  } catch (error) {
    // Якщо виникає помилка, повертаємо її текст.
...
function decodeURIComponentWrapper(encodedString) {
  ...
  try {
    // Спробуємо декодувати рядок
    return decodeURIComponent(encodedString);
  } catch (error) {
    // Якщо виникла помилка URIError, повертаємо повідомлення про помилку декодування URI
...
function findEvenNumber(numbers) {
  ...
  try {
    let evenNumber; // Ініціалізуємо змінну evenNumber без значення

    for (let number of numbers) {
      if (number % 2 === 0) {
        evenNumber = number;
        break; // Знашли перше число, що ділиться на 2 без остачі, і виходимо з циклу
      }
    }

    // Якщо знайдено число, повертаємо його
    if (evenNumber !== undefined) {
      return evenNumber;
    } else {
      // Якщо такого числа немає, генеруємо помилку
      throw new Error("У масиві немає чисел, що діляться на 2 без остачі!");
    }
  } catch (error) {
    // Обробляємо помилку і виводимо її текстове представлення
...
function validateUser(user) {
  ...
  try {
    ...
    // Виводимо повідомлення про помилку та причину помилки
    console.error(`${error.message} ${JSON.stringify(user)}`);
  }
}
...
function calculateSquareRoot(number) {
  ...
  try {
    ...
    // Повертаємо квадратний корінь з числа
...
function evaluateExpression(expression) {
  ...
  try {
    ...
    // Якщо була виявлена помилка, повертаємо повідомлення про помилку при виконанні функції eval
